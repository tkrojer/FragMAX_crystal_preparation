...
#
#    fieldnames = misc.get_shifter_csv_header()
...
#        d = misc.read_line_from_shifter_csv(logger, line)
#        if d:
...
#    df.to_csv(os.path.join(os.path.join(folder, '2-soak', soak_plate_name + '.csv')), index=False)
...
#    misc.backup_file(logger, os.path.join(folder, '1-inspect'), barcode + '_shifter.csv')
#    csv = ''
#    subwells = []
#    if int(subwell_a) != 0:
#        subwells.append('a')
